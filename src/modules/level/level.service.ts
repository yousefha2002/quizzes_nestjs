...
export class LevelService {
    ...
    async create(dto: CreateLevelDto) 
    {
        ...
        if (existing) {
            throw new BadRequestException('A level with this title already exists in the same category');
        }
    ...
    async update(id: number, dto: UpdateLevelDto) 
    {
        ...
        if (dto.title !== undefined) {
            ...
            if (existing) {
                throw new BadRequestException('Another level with this title already exists in the same category');
            }
    ...
    async publish(id: number) 
    {
        ...
        if (!level.quizzes || level.quizzes.length === 0) {
            throw new BadRequestException('Cannot publish level without at least one published and unfrozen quiz');
        }
