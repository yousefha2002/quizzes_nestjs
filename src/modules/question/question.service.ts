...
export class QuestionService {
  ...
  async createMany(dto: CreateQuestionsForQuizDto) {
    ...
    if (dto.question.answers.length < 2) {
      throw new BadRequestException('At least 2 answers are required.');
    }

    const hasCorrect = dto.question.answers.some((ans) => ans.isCorrect);
    if (!hasCorrect) {
    ...
    await this.answerService.createMany(question.id, dto.question.answers);
    ...
    if (hasAttempts) {
      ...
      if (dto.answers && dto.answers.length > 0) {
        throw new BadRequestException(
          'Cannot modify answers of a quiz with attempts',
        );
      }
      if (dto.title) {
      ...
      return {
        message:
          'Question title updated successfully. Answers cannot be changed because quiz has attempts.',
      };
    }
