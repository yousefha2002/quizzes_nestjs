...
export class AttemptService {
  ...
  async updateAttemptAnswer(
    ...
    await this.attemptAnswerService.updateAttemptAnswer(
      attemptId,
      questionId,
      answerId,
    );
  ...
  async submitAttempt(userId: number, attemptId: number) {
    ...
    if (percentage >= quiz.passScore) {
      const alreadyGotPoint = await this.pointsService.hasAlreadyGotPassPoints(
        userId,
        quiz.id,
      );
      if (!alreadyGotPoint) {
  ...
  async getUserAttemptsForQuiz(
    ...
    const totalAttempts = await this.attemptModel.count({
      where: {
        userId,
        quizId,
        status: { [Op.ne]: AttemptStatus.in_progress },
      },
    });
